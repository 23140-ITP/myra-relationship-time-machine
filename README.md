...
MYRA binds to localhost, and the browser never receives the Supermemory credential.
...
Supermemory owns ingestion, extraction, embeddings, semantic search, numeric filtering, and deletion. MYRA owns reviewed state plus deterministic timeline and Wrapped derivation. There is no browser credential, separate database, or cloud fallback.
...
- Provider-based extraction may send fixture text to the configured model provider. The verified demo configuration uses OpenRouter, so extraction is not fully offline.
